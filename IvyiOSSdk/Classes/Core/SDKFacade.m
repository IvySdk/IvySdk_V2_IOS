...
#import "SDKPayUtil.h"
#import <IvyiOSSdk/SDKDeviceUtil.h>
...
-(long)firstOpenTime
{
    if (_firstOpenTime <= 0) {
        @try {
            ...
            if (_firstOpenTime <= 0) {
                _firstOpenTime = [[NSDate date] timeIntervalSince1970];
                [[SDKCache cache] setObject:[NSNumber numberWithLong:_firstOpenTime] forKey:SDK_FIRST_OPEN];
            }
...
//                data = [SDKJSONHelper getJsonDataFrom:@"default" bundle:nil extension:@"json" AESKey:SDK_CONFIG_KEY];
        ...
        if ([_config objectForKey:@"enableAfAdPing"]) {
            _af_adrevenue = !![_config objectForKey:@"enableAfAdPing"];//!![_config objectForKey:@"af_ad_revenue"];
        } else {
            _af_adrevenue = TRUE;
        }
        // _af_adrevenue = !![_config objectForKey:@"enableAfAdPing"];//!![_config objectForKey:@"af_ad_revenue"];
...
- (void)_setConfig:(NSMutableDictionary *)config
{
    if(config) {
        ...
        NSString* api_top_user = [_config objectForKey:@"api.top_user_advalue"];
        if (api_top_user != nil) {
            [UacLTV setupToday];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(10 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [UacLTV checkAndUpdateUacTop:api_top_user];
            });
        }
    }
}
...
-(void)_syncConfig
{
    ...
    if(url != nil && url != NULL) {
        ...
        [[SDKNetworkHelper sharedHelper] GET:url parameters:nil jsonRequest:TRUE jsonResponse:TRUE success:^(id responseObject) {
            //请求成功
...
//                NSString *str = [[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding];
                @try {
                    ...
                    if (decrypt) {
                        NSMutableDictionary *defaultData = [SDKJSONHelper toArrayOrNSDictionary:decrypt];
                        if (defaultData && defaultData.count > 0) {
                            NSMutableDictionary* data = [defaultData objectForKey:@"default_json"];
...
//    }
    ...
    if (self.config) {
        ...
        [params setObject:[self getConfig:SDK_CONFIG_KEY_COUNTRY] forKey:@"country"];
        [params setObject:v_api forKey:@"data_version"];
        ...
        for (NSString *key in params) {
            _url = [_url stringByAppendingFormat:@"%@=%@&", key, [params objectForKey:key]];
        }
        _url = [_url substringToIndex:_url.length - 1];
        _url = [NSString stringWithFormat:@"%@://%@%@?%@", url.scheme, url.host, url.path, _url];
        return _url;
...
-(void)resetUIViewController
{
    if (_rootVC) {
        BOOL state = _rootVC.isViewLoaded && _rootVC.view.window;
        if (!state) {
...
-(void)_initAds
{
    [self resetUIViewController];
    if(_config) {
        if (_hasInitedAdConfig) {
            return;
        }
...
#endif
#if DEBUG
        //        GADMobileAds.sharedInstance.requestConfiguration.testDeviceIdentifiers = @[ @"62969efa3a933a5de0d997ef3387695d" ];
#endif
...
-(void)showDeliciousIconAd:(CGRect)frame configJson:(NSString *_Nullable)json
{
    ...
    if (_deliciousIconView) {
        ...
        [[SDKFacade sharedInstance] logAdImpression:[SDKConstants getAdTypeName:SDK_ADTYPE_GIFICON] adPos:@"icon" platform:@"our" mediation:@"our"];
    }
}
...
{
    if([self isAdsEnabled]) {
        [self logEvent:@"sdk_ad_fired" withData:@{@"event_src":@"sdk", @"ad_format":@"banner"}];
...
{
    if([self isAdsEnabled]) {
        [self logEvent:@"sdk_ad_fired" withData:@{@"event_src":@"sdk", @"ad_format":@"interstitial"}];
        __weak SDKInterstitialModule *module = [self getAdModule:SDK_ADTYPE_INTERTITIAL withTag:tag];
        if(module) {
...
{
    [self logEvent:@"sdk_ad_fired" withData:@{@"event_src":@"sdk", @"ad_format":@"rewarded"}];
    __weak SDKVideoModule *module = [self getAdModule:SDK_ADTYPE_VIDEO withTag:tag];
    if(module) {
...
-(void)showIconAd:(float)width xPercent:(float)xPercent yPercent:(float)yPercent
{
    if([self changeIcon:[self pickAvailableApp:SDK_ADTYPE_ICON]]) {
        ...
        [[SDKFacade sharedInstance] logAdImpression:[SDKConstants getAdTypeName:SDK_ADTYPE_ICON] adPos:@"icon" platform:@"our" mediation:@"our"];
...
-(void)application:(UIApplication *)application performActionForShortcutItem:(UIApplicationShortcutItem *)shortcutItem completionHandler:(void (^)(BOOL))completionHandler
{
    @try {
        ...
        if (type && _sdkDelegate) {
            [_sdkDelegate on3DTouchEvent:type];
        }
    } @catch (NSException *exception) {
                    ...
                    if (arr && arr.count > 0) {
                        for (int i=0; i<arr.count; i++) {
                            ...
                            if (self->_opCount == value) {
                                [[SDKFacade sharedInstance] logEvent:[NSString stringWithFormat:@"op_%ld", self->_opCount]];
                                break;
                            }
                        }
                    }
...
-(void)sendToken:(NSString *)token completeBlock:(void(^)(NSDictionary* data))completeBlock
{
    ...
    NSURLSessionDataTask* dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if (error) {
            ...
            NSError* resErr;
            NSMutableDictionary *resDict = [NSJSONSerialization JSONObjectWithData:data options:kNilOptions error:&resErr];
            result = [[NSDictionary alloc] initWithDictionary:resDict];
            if (completeBlock) {
...
- (void)callClientRequestATT
{
    if (_adDelegate != nil) {
        [_adDelegate onHaveToRequestATT];
    }
}
...
- (void)isAppleSigned
{
    NSDictionary * info = [self getSignedAppleInfo];
    ...
    if (info.count > 0) {
        ...
        if(idToken == nil){
            if (_snsDelegate && [_snsDelegate respondsToSelector:@selector(signInAppleFailure:)]) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self->_snsDelegate signInAppleFailure:@"invalid token"];
                });
            }
        } else {
            [self verifyAppleSignIn:idToken userId:user];
        }
    } else {
...
-(void)verifyAppleSignIn:(NSString*)idToken userId:(NSString*)userId
{
    NSString* appid =[[SDKFacade sharedInstance] getConfig:SDK_CONFIG_KEY_APP_ID];
    if (appid == nil) {
        if (_snsDelegate && [_snsDelegate respondsToSelector:@selector(signInAppleFailure:)]) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self->_snsDelegate signInAppleFailure:@"invalid appid"];
            });
        }
        return;
    }
    NSString* url = @"https://verify.ivymobile.com/api/external/v1/login/apple";
    NSMutableDictionary* params = [[NSMutableDictionary alloc] init];
...
-(void)_initPayment
{
    ...
    if(_config) {
        ...
        if (payment && [payment count] > 0) {
            ...
            if (_paymentVerifyUrl != nil) {
                sdkPayUtil = [[SDKPayUtil alloc] initWithVerifyUrl:_paymentVerifyUrl];
            }
...
-(void)paySuccess:(int)paymentId payload:(NSString *)payload productIdentifier:(NSString *)productIdentifier transactionIdentifier:(NSString *)transactionIdentifier orderInfo:(nullable NSString *)orderInfo merchantTransactionId:(nullable NSString*)merchantTransactionId
{
    @try {
        [SDKHelper hideLoading];
        ...
        if (_paymentDelegate) {
            if (payload) {
                if (orderInfo) {
                    if ([_paymentDelegate respondsToSelector:@selector(onPaymentSuccess:payload:merchantTransactionId:orderInfo:)]) {
                        [_paymentDelegate onPaymentSuccess:paymentId payload:payload merchantTransactionId:merchantTransactionId orderInfo:orderInfo];
                    }
                } else {
                    if ([_paymentDelegate respondsToSelector:@selector(onPaymentSuccess:payload:merchantTransactionId:)]) {
                        [_paymentDelegate onPaymentSuccess:paymentId payload:payload merchantTransactionId:merchantTransactionId];
                    }
                }
            } else {
                if([_paymentDelegate respondsToSelector:@selector(onPaymentSuccess:merchantTransactionId:)]) {
                    [_paymentDelegate onPaymentSuccess:paymentId merchantTransactionId:merchantTransactionId];
                }
            }
...
//    if (_force_check < 2) {
        ...
        if (arr && arr.count > 0) {
            for (NSDictionary *obj in arr) {
                NSString *merchantTransactionId = [obj objectForKey:@"merchantTransactionId"];
            ...
            @try {
                if (data && data.count > 0) {
                    for(NSDictionary* item in data){
                        NSMutableDictionary* response = [[NSMutableDictionary alloc] init];
                        [response setObject:@(0) forKey:@"status"];
                        NSNumber* paymentId = [item objectForKey:@"pay_id"];
                        NSString* payload = [item objectForKey:@"payload"];
                        NSString* productIdentifier = [item objectForKey:@"product_identifier"];
                        NSString* transactionIdentifier = [item objectForKey:@"transaction_identifier"];
                        NSString* merchant_transaction_id = [item objectForKey:@"merchant_transaction_id"];
                        [self verifyPaymentResponse:response paymentId:paymentId.intValue payload:payload productIdentifier:productIdentifier transactionIdentifier:transactionIdentifier merchantTransactionId:merchant_transaction_id];
                    }
                }
            } @catch (NSException *exception) {
                NSLog(@"PayUtil -- query unshipping goods failed");
            }
        }];
    }
...
-(void)encodePayload:(nonnull NSString*)merchant_transaction_id payload:(nullable NSString*)payload
{
    if (payload) {
        [[SDKCache cache] setObject:payload forKey:merchant_transaction_id];
    }
}

-(nullable NSString *)decodePayload:(nonnull NSString*)merchant_transaction_id
{
    @try {
        if ([[SDKCache cache] containsObjectForKey:merchant_transaction_id]) {
            NSString* payload = (NSString *)[[SDKCache cache] objectForKey:merchant_transaction_id];
            return payload;
        }
    } @catch (NSException *exception) {
...
- (void)shippingGoods:(NSString *)merchant_transaction_id{
    if (!merchant_transaction_id) {
        if (_paymentDelegate && [_paymentDelegate respondsToSelector:@selector(onShippingGoods:status:)]) {
            [_paymentDelegate onShippingGoods:merchant_transaction_id status:false];
        }
        return;
    }
    if (sdkPayUtil) {
        [sdkPayUtil shippngGoods:merchant_transaction_id callback:^(BOOL status) {
            if (self->_paymentDelegate && [self->_paymentDelegate respondsToSelector:@selector(onShippingGoods:status:)]) {
                [self->_paymentDelegate onShippingGoods:merchant_transaction_id status:true];
            }
        }];
    } else {
        if (_paymentDelegate && [_paymentDelegate respondsToSelector:@selector(onShippingGoods:status:)]) {
...
- (void)getNotificationPermissionState
{
    ...
    [center getNotificationSettingsWithCompletionHandler:^(UNNotificationSettings * _Nonnull settings) {
        if (settings.authorizationStatus == UNAuthorizationStatusAuthorized) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self->_sdkDelegate != nil) {
                    [self->_sdkDelegate onNotificationPermissionState:1];
                }
            });
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self->_sdkDelegate != nil) {
                    [self->_sdkDelegate onNotificationPermissionState:0];
                }
            });
        } else if(settings.authorizationStatus == UNAuthorizationStatusNotDetermined){
            NSLog(@"Notifications permission not determined");
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self->_sdkDelegate != nil) {
                    [self->_sdkDelegate onNotificationPermissionState:2];
                }
            });
        } else if(settings.authorizationStatus == UNAuthorizationStatusProvisional){
            NSLog(@"Notifications are provisional");
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self->_sdkDelegate != nil) {
                    [self->_sdkDelegate onNotificationPermissionState:0];
                }
...
-(void) authFirestoreCallBack:(FIRUser* _Nullable) authUser{
    for (id<FIRUserInfo> firUserInfo in [authUser providerData]) {
        if ([[firUserInfo providerID] isEqualToString:@"apple.com"]) {
            NSString* me = firUserInfo ? [NSString stringWithFormat:@"{\"id\":\"%@\", \"name\":\"%@\", \"picture\":\"%@\"}", firUserInfo.uid, firUserInfo.displayName, firUserInfo.photoURL] : @"{}";
            [self->_firestoreDelegate onFirestoreConnected: me];
            return;
        }
    }
...
-(void) linkFirestoreAfterAppleSignIn {
#ifdef FIREBASE_PLUS
    FIRUser* currentUser = [[FIRAuth auth] currentUser];
    NSString* isNewUser = (NSString*)[[SDKCache cache] objectForKey:@"newLoginUser"];
    //不要重新登录时
    if((currentUser != nil)&&![isNewUser isEqualToString:@"true"]) {
        NSString* appleId = [self getSignedAppleUID];
        if(![appleId isEqualToString:@""]){
            //本地存储了appleid
            NSString* me = [NSString stringWithFormat:@"{\"id\":\"%@\"}", appleId];
            [self->_firestoreDelegate onFirestoreConnected: me];
        }else {
            //本地未存储appleid
            [self->_firestoreDelegate onFirestoreConnectError:@""];
        }
        return;
    }
    ...
    //获取apple账号登录凭证
    NSDictionary * dict = [self getSignedAppleInfo];
    NSString* idToken = [dict objectForKey:@"idToken"];
    NSString* tRawNonce = [dict objectForKey:@"nonce"];
    //检查idtoken和nonce
    if((idToken == nil)||(tRawNonce == nil)) {
        [_firestoreDelegate onFirestoreConnectError:@"Apple not SignIn!"];
        return;
    }
    
    //使用本次apple登录的凭证建立firebase的新证书
    FIROAuthCredential* credential = [FIROAuthProvider appleCredentialWithIDToken:idToken rawNonce:self.currentNonce fullName:self.appleFullName];
    [[FIRAuth auth] signInWithCredential:credential completion:^(FIRAuthDataResult * _Nullable authResult,
                                                                 NSError * _Nullable error){
        // User successfully signed in. Get user data from the FIRUser object
        if (authResult == nil || error != nil) {
            if (error != nil) {
                [self->_firestoreDelegate onFirestoreConnectError:error.description];
            } else {
                [self->_firestoreDelegate onFirestoreConnectError:@"Unknown Error"];
            }
            return;
        }
        
        //firebase store登陆成功时移除新用户登录标志
        [[SDKCache cache] removeObjectForKey:@"newLoginUser"];
        
        FIRUser* authUser = authResult.user;
        [self authFirestoreCallBack:authUser];
    }];
#endif
}
...
-(void) deleteAppleCountFromFirestore {
    ...
    //获取apple账号登录凭证
    NSDictionary * dict = [self getSignedAppleInfo];
    NSString* authCode = [dict objectForKey:@"authCode"];
    if((authCode == nil)||([authCode isEqualToString:@""])){
        return;
    }
    [[FIRAuth auth] revokeTokenWithAuthorizationCode:authCode completion:^(NSError * _Nullable error) {
        if(error != nil) {
            DLog(@"revokeTokenWithAuthorizationCode error:%@", error.description);
        }
        [currentUser deleteWithCompletion:^(NSError * _Nullable error) {
            if(error != nil) {
                DLog(@"deleteWithCompletion error:%@", error.description);
            }
        }];
    }];
#endif
...
-(void) signInAfterFacebookSignedIn
{
#ifdef FIREBASE_PLUS
    FIRUser* currentUser = [[FIRAuth auth] currentUser];
    ...
    if (currentUser == nil || [currentUser isAnonymous]) {
        [[FIRAuth auth] signInWithCredential:credential
            ...
            for (id<FIRUserInfo> firUserInfo in [authUser providerData]) {
                if ([[firUserInfo providerID] isEqualToString:@"facebook.com"]) {
                    NSString* me = firUserInfo ? [NSString stringWithFormat:@"{\"id\":\"%@\", \"name\":\"%@\", \"picture\":\"%@\"}", firUserInfo.uid, firUserInfo.displayName, firUserInfo.photoURL] : @"{}";
                    [self->_firestoreDelegate onFirestoreConnected: me];
                    return;
                }
        ...
        // 如果当前账号是apple, link过去
        BOOL isFacebookProvider = NO;
        for (id firUserInfo in [currentUser providerData]) {
            if ([[firUserInfo providerID] isEqualToString:@"facebook.com"]) {
                isFacebookProvider = YES;
                break;
            }
        }
        ...
        if (isFacebookProvider) {
            [[FIRAuth auth] signInWithCredential:credential
                ...
                // User successfully signed in. Get user data from the FIRUser object
                if (authResult == nil || error != nil) {
                    [self->_firestoreDelegate onFirestoreConnectError:@"Login Facebook Failed"];
                    return;
                }
            ...
            [currentUser linkWithCredential:credential completion:^(FIRAuthDataResult * _Nullable authResult, NSError * _Nullable error) {
                // 切换账号
                if (authResult == nil || error != nil) {
                    [self->_firestoreDelegate onFirestoreConnectError:@"Link to account failed"];
                    return;
                }
...
/**
 * 登入Facebook并将Facebook账号link到当前账号
 */
-(void) initializeAndLinkFacebookAfterSignIn
{
    ...
    if ([self isLogin]) {
        [self signInAfterFacebookSignedIn];
        return;
    }
    
    [self login:^(NSError * _Nullable error) {
        if (error) {
            [self->_firestoreDelegate onFirestoreConnectError:[error localizedDescription]];
        } else {
            [self signInAfterFacebookSignedIn];
        }
    }];
...
-(void)signOutFirestore
{
    ...
    FIRUser* currentUser = [[FIRAuth auth] currentUser];
    if (currentUser != nil && [currentUser isAnonymous]) {
        [_fireStore signOutFirestore: _firestoreDelegate];
    }
#endif
}
...
{
#ifdef FIREBASE_PLUS
    [_firebaseDatabase sendChat:database path:path jsonData:jsonData];
#endif
}
...
{
#ifdef FIREBASE_PLUS
    [_firebaseDatabase listen:database path:path handler:_firebaseDatabaseDelegate];
#endif
}

-(nullable NSDictionary *)getLocalNotificationData
{
    return (NSDictionary *)[[SDKCache cache] objectForKey:SDK_LOCALNOTIFICATION_USERINFO];
}

-(nullable NSString *)getLocalNotificationDataJson
{
    return [SDKJSONHelper toJSONString:[[SDKCache cache] objectForKey:SDK_LOCALNOTIFICATION_USERINFO]];
}
...
- (void)pushLocalDelayNotification:(NSString *)key title:(NSString *)title msg:(NSString *)msg pushTime:(NSTimeInterval)seconds interval:(NSTimeInterval)interval repeat:(BOOL)repeat useSound:(BOOL)useSound soundName:(NSString *)soundName userInfo:(NSDictionary *)userInfo
{
    [self cancelLocalNotification:key];
    [self cancelLocalNotification:[key stringByAppendingString:@"_interval"]];
    //设置通知推送携带的参数，一般用于点击通知打开指定控制器页面
    NSMutableDictionary *_userInfo = [[NSMutableDictionary alloc] init];
    [_userInfo setObject:key forKey:@"key"];
    if (userInfo) {
        [_userInfo setValuesForKeysWithDictionary:userInfo];
    }
    //    interval = interval <= 0 ? SDK_INTERVAL_DAY : interval;
    //    if (@available(iOS 10.0, *)) {
    //创建本地通知对象
    UNUserNotificationCenter *center = [UNUserNotificationCenter currentNotificationCenter];
    ...
    NSDate *fireDate = [now dateByAddingTimeInterval:seconds];
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDateComponents* dateComps = [calendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond fromDate:fireDate];
    
    UNCalendarNotificationTrigger *calendarTrigger = [UNCalendarNotificationTrigger triggerWithDateMatchingComponents:dateComps repeats:NO];
    UNNotificationRequest *fireRequest = [UNNotificationRequest requestWithIdentifier:key content:content trigger:calendarTrigger];
    [center addNotificationRequest:fireRequest withCompletionHandler:^(NSError * _Nullable error) {
        
    }];
    if (repeat) {
        UNTimeIntervalNotificationTrigger *intervalTriger = [UNTimeIntervalNotificationTrigger triggerWithTimeInterval:interval repeats:YES];
        UNNotificationRequest *intervalRequest = [UNNotificationRequest requestWithIdentifier:[key stringByAppendingString:@"_interval"] content:content trigger:intervalRequest];
        [center addNotificationRequest:intervalRequest withCompletionHandler:nil];
    }
    
    //    }
}

-(void)pushLocalNotification:(NSString *)key title:(NSString *)title msg:(NSString *)msg pushTime:(NSTimeInterval)seconds interval:(SDK_INTERVAL)interval repeat:(BOOL)repeat useSound:(BOOL)useSound soundName:(NSString *)soundName userInfo:(NSDictionary *)userInfo
{
    NSDate *date = nil;
    if (seconds <= 0) {
        date = [NSDate dateWithTimeIntervalSinceNow:0];
    } else if (seconds < [NSDate date].timeIntervalSince1970) {
        date = [NSDate dateWithTimeIntervalSinceNow:seconds];
    } else {
        date = [NSDate dateWithTimeIntervalSince1970:seconds];
    }
    [self pushLocalNotification:key title:title msg:msg fireDate:date interval:(int)interval repeat:repeat useSound:useSound soundName:soundName userInfo:userInfo];
}

-(void)pushLocalNotification:(NSString *)key title:(NSString *)title msg:(NSString *)msg dateStr:(NSString *)dateStr interval:(SDK_INTERVAL)interval repeat:(BOOL)repeat useSound:(BOOL)useSound soundName:(NSString *)soundName userInfo:(NSDictionary *)userInfo
{
    //处理时间字符串
    ...
    [self pushLocalNotification:key title:title msg:msg fireDate:fireDate interval:(int)interval repeat:repeat useSound:useSound soundName:soundName userInfo:userInfo];
}

-(void)pushLocalNotification:(nonnull NSString *)key title:(nonnull NSString *)title msg:(nonnull NSString *)msg fireDate:(NSDate *)date interval:(int)interval repeat:(BOOL)repeat useSound:(BOOL)useSound soundName:(NSString *)soundName userInfo:(nullable NSDictionary *)userInfo
{
    [self cancelLocalNotification:key];
    //设置通知推送携带的参数，一般用于点击通知打开指定控制器页面
    NSMutableDictionary *_userInfo = [[NSMutableDictionary alloc] init];
    ...
    if (@available(iOS 10.0, *)) {
        ...
        // repeat，是否重复，如果重复的话interval时间必须大于60s，要不会报错
        if (repeat) {
            NSDateComponents * comp = [[NSCalendar currentCalendar] components:NSCalendarUnitYear | NSCalendarUnitSecond | NSCalendarUnitMinute | NSCalendarUnitMonth | NSCalendarUnitHour | NSCalendarUnitDay fromDate:date];
            NSDateComponents *components = [[NSDateComponents alloc] init];
            components.second = comp.second;
            if (interval == SDK_INTERVAL_MINUTE) {
                //每分钟循环
            } else if (interval == SDK_INTERVAL_HOUR) {
                //每小时循环
                components.minute = comp.minute;
            } else if (interval == SDK_INTERVAL_DAY) {
                //每天循环
                components.minute = comp.minute;
                components.hour = comp.hour;
            } else if (interval == SDK_INTERVAL_WEEKDAY) {
                //每周循环
                components.minute = comp.minute;
                components.hour = comp.hour;
                components.weekday = comp.weekday;
            } else if (interval == SDK_INTERVAL_MONTH) {
                //每月循环
                components.minute = comp.minute;
                components.hour = comp.hour;
                components.day = comp.day;
                components.month = comp.month;
            } else if (interval == SDK_INTERVAL_YEAR) {
                //每年循环
                components.minute = comp.minute;
                components.hour = comp.hour;
                components.day = comp.day;
            ...
            //不循环，定时推送
            trigger = [UNTimeIntervalNotificationTrigger triggerWithTimeInterval:date.timeIntervalSinceNow+1 repeats:NO];
        }
        content.userInfo = _userInfo;
        UNNotificationRequest *notificationRequest = [UNNotificationRequest requestWithIdentifier:key content:content trigger:trigger];
        [center addNotificationRequest:notificationRequest withCompletionHandler:^(NSError * _Nullable error) {
        }];
    } else {
        UILocalNotification *localNoti  = [[UILocalNotification alloc] init];
        [localNoti setTimeZone:[NSTimeZone defaultTimeZone]];//设置时区
        //通知触发时间
        ...
        localNoti.repeatInterval = interval;
        localNoti.userInfo = _userInfo;
        //调用通知
        [[UIApplication sharedApplication] scheduleLocalNotification:localNoti];
    }
}

-(void)pushRemoteNotification:(nullable NSString *)key title:(NSString *)title content:(NSString *)content pushTime:(long)pushTime useLocalTimeZone:(BOOL)useLocalTimeZone facebookIds:(NSString *)facebookIds uuids:(NSString *)uuids topic:(nullable NSString *)topic iosBadge:(int)iosBadge useSound:(BOOL)useSound soundName:(NSString *)soundName extraData:(NSDictionary *)data
{
    if(_hasInit) {
        if(_pushes && _pushes.count > 0) {
            for (int i=0,n=(int)_pushes.count; i<n; i++) {
                SDKPush *push = [_pushes objectAtIndex:i];
                [push pushMessage:key title:title content:content pushTime:pushTime useLocalTimeZone:useLocalTimeZone facebookIds:facebookIds uuids:uuids topic:topic iosBadge:iosBadge useSound:useSound soundName:soundName extraData:data];
            }
        }
    }
}
...
//-(void)pushRepeatRemoteNotification:(NSString *)messageId title:(NSString *)title content:(NSString *)content pushTime:(long)pushTime useLocalTimeZone:(BOOL)useLocalTimeZone facebookIds:(NSString *)facebookIds uuids:(NSString *)uuids topic:(nullable NSString *)topic iosBadge:(int)iosBadge useSound:(BOOL)useSound soundName:(NSString *)soundName extraData:(NSDictionary *)data repeatInterval:(int)repeatInterval
//{
//    if(_hasInit) {
//        if(_pushes && _pushes.count > 0) {
//            for (int i=0,n=(int)_pushes.count; i<n; i++) {
//                SDKPush *push = [_pushes objectAtIndex:i];
//                [push pushRepeatMessage:messageId title:title content:content pushTime:pushTime useLocalTimeZone:useLocalTimeZone facebookIds:facebookIds uuids:uuids topic:topic iosBadge:iosBadge useSound:useSound soundName:soundName extraData:data repeatInterval:repeatInterval];
//            }
//        }
//    }
//}
//
//-(void)cancelRepeatRemoteNotification:(NSString *)messageId
//{
//    if(_hasInit) {
//        if(_pushes && _pushes.count > 0) {
//            for (int i=0,n=(int)_pushes.count; i<n; i++) {
//                SDKPush *push = [_pushes objectAtIndex:i];
//                [push cancelRepeatPush:messageId];
//            }
//        }
//    }
//}
...
- (void)logAppsflyrEvent:(NSString *)eventId
{
    if (_iPad && isPad) {
        eventId = [NSString stringWithFormat:@"ipad_%@", eventId];
    }
    [self checkAutoEvent:eventId];
    SDKAnalyse *_analyse = [self getAnalyse:SDK_ANALYSE_APPSFLYER];
    if(_analyse) {
        [_analyse logEvent:eventId];
    }
}

- (void)logAppsflyrEvent:(NSString *)eventId withData:(NSDictionary<NSString *,id> *)data
{
    if (_iPad && isPad) {
        eventId = [NSString stringWithFormat:@"ipad_%@", eventId];
    }
...
-(void)logAdRevenue:(nonnull NSString *)adClassName mediationType:(SDK_MEDIATION_TYPE)mType adType:(nonnull NSString *)adType adUnit:(nonnull NSString *)adUnit placement:(nonnull NSString *)placement country:(nonnull NSString *)country currency:(nonnull NSString *)currency value:(NSNumber * _Nonnull)value
{
    ...
    double totalRevenue = [self mmGetDoubleValue: KEY_TOTAL_AD_REVENUE defaultValue:0.0] + value.doubleValue;
    [self mmSetDoubleValue:KEY_TOTAL_AD_REVENUE value:totalRevenue];
    [UacLTV checkUacLtvConversion:value.doubleValue totalRevenue:totalRevenue];
    [UacLTV checkFirstThreeDaysLTV:totalRevenue];
#ifdef APPSFLYER
    if (_af_adrevenue) {
//        AFAdRevenueData* revenueData = [[AFAdRevenueData alloc] initWithMonetizationNetwork:adClassName mediationNetwork:AppsFlyerAdRevenueMediationNetworkTypeApplovinMax currencyIso4217Code:currency eventRevenue:value];
//        [[AppsFlyerLib shared] logAdRevenue:revenueData additionalParameters:@{@"country" : country, @"ad_unit" : adUnit, @"ad_type" : adType, @"ad_format" : adType, @"placement" : placement, @"ad_network" : adClassName}];
        [[AppsFlyerLib shared] logEvent:@"af_ad_revenue" withValues:@{
            @"ad_network": adClassName,
            @"ad_format": adType,
            @"ad_type": adType,
            @"placement" : placement,
    ...
    //    }
    //#endif
    [self logIvyEvent:@"sdk_ad_revenue" withData:@{
        @"ad_network": adClassName,
        @"ad_format": adType,
        @"ad_type": adType,
...
-(void)logAdImpression:(nonnull NSString *)adType adPos:(nonnull NSString *)adPos platform:(nonnull NSString *)platform mediation:(nullable NSString *)mediation
{
    if(mediation == nil){
        mediation = @"";
    }
    NSString * netStatus = [[SDKFacade sharedInstance] isNetworkConnected] ? @"1" : @"0";
    NSDictionary *params = @{@"ad_type" : adType, @"ad_pos" : (adPos ? adPos : @"null"), @"platform" : platform, @"mediation" : mediation, @"sdkNetStatus":netStatus, @"fst": [NSString stringWithFormat:@"%ld", self.firstOpenTime]};
    BOOL isCross = [platform isEqualToString:@"our"] || [platform isEqualToString:@"delicious"];
    BOOL isVideo = [adType isEqualToString:@"video"];
    BOOL isInter = [adType isEqualToString:@"interstitial"];
    NSString *eventId = nil;
...
-(void)logPay:(int)paymentId productIdentifier:(NSString *)productIdentifier transactionIdentifier:(NSString *)transactionIdentifier
{
    ...
    if(_analyses && _analyses.count > 0) {
        ...
        for (SDKAnalyse *_analyse in _analyses) {
            ...
            if (@available(iOS 10.0, *)) {
                [_analyse logPay:productIdentifier price:price name:title number:1 currency:currency first_purchase:isFirstPurchase];
            } else {
                // Fallback on earlier versions
                [_analyse logPay:productIdentifier price:price name:title number:1 currency:currency first_purchase:isFirstPurchase];
            }
...
-(nullable NSString *)getConfig:(SDK_CONFIG_ID)cid;
{
    ...
    switch (cid) {
        ...
        case SDK_CONFIG_KEY_FIREBASE_USERID:
            ...
            break;
        case SDK_CONFIG_KEY_APPSTORE_ID:
            if(self.config){
                result = [self.config objectForKey:@"appstoreid"];
            }
            break;
        case SDK_CONFIG_KEY_IDFV:
            result = [SDKHelper getIDFVString];
            break;
        default:
            break;
    }
...
//
    ...
    float statusBarHeight = [self getStatusBarHeight];
...
- (void)mmSetDoubleValue:(NSString *)key value:(double)value
{
    if (_mmEnableConfuse) {
        NSNumber *confuse = [self mmResetConfuseNumber:key size:100];
        value *= confuse.intValue;
    }
    [[SDKCache cache] setObject:[NSNumber numberWithDouble:value] forKey:key];
}
- (double)mmGetDoubleValue:(NSString *)key defaultValue:(double)defaultValue
{
    if ([[SDKCache cache] containsObjectForKey:key]) {
        NSNumber *value = (NSNumber *)[[SDKCache cache] objectForKey:key];
        double num = value != nil ? value.doubleValue : defaultValue;
        if (_mmEnableConfuse) {
            NSNumber *confuse = [self mmGetConfuseNumber:key];
            num /= confuse.doubleValue;
        }
        return num;
    }
    return defaultValue;
...
+ (int)checkIDFAAuthorizationStatus
{
    if (@available(iOS 14, *)) {
...
-(void)cloudFunc:(NSString *)funcKey params:(NSDictionary *)params
{}

-(void)cloudFuncWithoutLogin:(NSString *)funcKey params:(NSDictionary *)params
{}

-(void)idCardVerify:(NSString *)name card:(NSString *)card
{}

- (void)addShortcut:(NSString *)type title:(NSString *)title subtitle:(NSString *)subtitle icon:(NSString *)icon
{
    UIApplication* application = [UIApplication sharedApplication];
    UIApplicationShortcutIcon* shortcutIcon = [UIApplicationShortcutIcon iconWithTemplateImageName:icon];
    UIApplicationShortcutItem *item = [[UIApplicationShortcutItem alloc] initWithType:type localizedTitle:title localizedSubtitle:subtitle icon:shortcutIcon userInfo:nil];
    if (application.shortcutItems != nil) {
        NSMutableArray* array = [[NSMutableArray alloc] init];
        for(id obj in application.shortcutItems){
            [array addObject:obj];
        }
...
- (void)deleteShortcut:(NSString *)type
{
    UIApplication* application = [UIApplication sharedApplication];
    if (application.shortcutItems != nil) {
        NSMutableArray* array = [NSMutableArray init];
        for(UIApplicationShortcutItem* obj in application.shortcutItems){
            if (obj.type == type) {
                continue;
            }
            [array addObject:obj];
...
+(void)openFacebook:(NSString *)pageId
{
    @try {
        if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"fb://"]]) {
            // Facebook App已安装，可以尝试打开指定主页
            NSString *facebookPageURLString = [NSString stringWithFormat:@"fb://profile/%@", pageId];
            NSURL *facebookPageURL = [NSURL URLWithString:facebookPageURLString];
            
            if ([[UIApplication sharedApplication] canOpenURL:facebookPageURL]) {
                // 可以打开Facebook App并跳转
                [[UIApplication sharedApplication] openURL:facebookPageURL options:@{} completionHandler:nil];
            } else {
                // 无法通过URL Scheme打开，可能是FB App版本不支持或未安装，采用网页链接替代
                NSString *webURLString = [NSString stringWithFormat:@"https://www.facebook.com/profile.php?id=%@", pageId];
...
+ (void)getApp:(NSString *)appstoreId
{
    if(appstoreId != NULL){
        [[SDKHelper sharedHelper] openAppStoreId:appstoreId];
    }
}

- (NSString *)getAfInviteId
{
#ifdef APPSFLYER
    @try {
        NSString * af_invite_id = (NSString *)[[SDKCache cache] objectForKey:@"af_invite_id"];
        return af_invite_id;
    } @catch (NSException *exception) {
...
- (void)inviteUserByAf:(NSString *)currentUserId
{
#ifdef APPSFLYER
    [[SDKCache cache] setObject:currentUserId forKey:@"af_invite_current_user_id"];
    [AppsFlyerShareInviteHelper generateInviteUrlWithLinkGenerator:^AppsFlyerLinkGenerator * _Nonnull(AppsFlyerLinkGenerator * _Nonnull generator) {
        [generator setChannel:@"ios_user_invite"];
        [generator setCampaign:@"user_invite"];
        [generator addParameterValue:currentUserId forKey:@"deep_link_sub1"];
        NSString* appId = [self getConfig:SDK_CONFIG_KEY_APP_ID];
        [generator addParameterValue:appId forKey:@"deep_link_sub2"];
        [generator addParameterValue:currentUserId forKey:@"af_sub1"];
        return generator;
    } completionHandler:^(NSURL * _Nullable url) {
        NSLog(@"Appsflyer DeepLink generator response:%@",url);
        if (url != nil) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [SDKHelper share:@"invite" image:nil url:url completeHandler:nil];
            });
        } else {
        }
    }];
...
{
    if (degree == 0) {
        [SDKHapticHelper generateFeedback:FeedbackType_Impact_Light];
    } else if(degree == 1){
        [SDKHapticHelper generateFeedback:FeedbackType_Impact_Medium];
    } else if(degree == 2){
        [SDKHapticHelper generateFeedback:FeedbackType_Impact_Heavy];
    } else {
        [SDKHapticHelper generateFeedback:FeedbackType_Impact_Light];
    }
}
