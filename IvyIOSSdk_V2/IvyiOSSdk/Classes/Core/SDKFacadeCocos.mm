...
-(void)onNotificationPermissionState:(int)status
{
    if (_delegate)
    {
        @try {
            _delegate->onNotificationPermissionState(status);
        } @catch (NSException *exception) {
            DLog(@"Exception: %@", [exception description]);
        } @catch (NSError *error) {
            DLog(@"Error: %@", [error localizedDescription]);
...
-(void)on3DTouchEvent:(NSString *)type
{
    if (_delegate)
    {
        @try {
            type = type ? type : @"{}";
            const char * extra = [type UTF8String];
            _delegate->on3DTouchEvent(extra);
        } @catch (NSException *exception) {
            DLog(@"Exception: %@", [exception description]);
        } @catch (NSError *error) {
            DLog(@"Error: %@", [error localizedDescription]);
...
void SDKFacadeCocos::getNotificationPermissionState(){
    [[SDKFacade sharedInstance] getNotificationPermissionState];
}

void SDKFacadeCocos::requestNotificationPermission(){
    [[SDKFacade sharedInstance] requestNotificationPermission];
}

void SDKFacadeCocos::openNotificationSettings(){
...
void SDKFacadeCocos::pushLocalDelayNotification(const char* _Nonnull key, const char* _Nonnull title, const char* _Nonnull msg, long pushTime, long interval, bool repeat, bool useSound, const char* _Nullable soundName, const char* _Nonnull userInfo)
{
    NSString *objc_key = [NSString stringWithUTF8String:key];
...
int SDKFacadeCocos::checkIDFAAuthorizationStatus(){
...
void SDKFacadeCocos::addShortcut(const char* _Nonnull type, const char* _Nonnull title, const char* _Nullable subtitle,  const char* _Nullable icon){
    [[SDKFacade sharedInstance] addShortcut:[NSString stringWithUTF8String:type] title:[NSString stringWithUTF8String:title] subtitle:[NSString stringWithUTF8String:subtitle] icon:[NSString stringWithUTF8String:icon]];
}

void SDKFacadeCocos::deleteShortcut(const char* _Nonnull type){
    [[SDKFacade sharedInstance] deleteShortcut:[NSString stringWithUTF8String:type]];
}
...
void SDKFacadeCocos::openFacebook(const char * _Nonnull pageId){
    if (pageId != NULL) {
        [SDKFacade openFacebook:[NSString stringWithUTF8String:pageId]];
    }
}

void SDKFacadeCocos::vibrate(int degree){
