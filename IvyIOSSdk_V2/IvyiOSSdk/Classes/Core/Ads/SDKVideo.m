...
-(void)adReward
{
    ...
    int playDuration = CACurrentMediaTime() - _startTime;
    NSString * netStatus = [[SDKFacade sharedInstance] isNetworkConnected] ? @"1" : @"0";
    NSDictionary *params = @{@"ad_format" : [SDKConstants getAdTypeName:self->_adType], @"mediation" : self->mediation, @"ad_placement": self->_adId, @"aad_network": self->_platform, @"sdkNetStatus": netStatus, @"ad_duration": [NSNumber numberWithInt:playDuration]};
    [[SDKFacade sharedInstance] logEvent:@"video_completed" withData:params];
    [[SDKFacade sharedInstance] logEvent:@"sdk_ad_rewarded" withData:params];
