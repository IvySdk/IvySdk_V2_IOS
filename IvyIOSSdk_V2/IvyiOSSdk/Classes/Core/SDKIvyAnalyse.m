...
-(void)setup:(nonnull NSDictionary *)conf
{
    ...
    NSString *roleId = [[SDKFacade sharedInstance] getConfig:SDK_CONFIG_KEY_UUID];//[SDKFacade sharedInstance].userId;
    ...
    [_ourEventParams setObject:[[SDKFacade sharedInstance] getConfig:SDK_CONFIG_KEY_UUID] forKey:@"uuid"];
...
{
    if (first_purchase) {
        [self logEvent2Our:@"iap_purchased" withData:@{@"revenue":price, @"itemid":itemName, @"catalog":@"first_purchase"}];
    } else {
