...
@private
    ...
    NSTimeInterval _rewardedLoadStartTime;
}
        ...
        if (_isRewardInterstitial) {
            ...
            [GADRewardedInterstitialAd loadWithAdUnitID:_adId request:request completionHandler:^(GADRewardedInterstitialAd* _Nullable rewardedInterstitialAd, NSError* _Nullable error) {
                if (!error) {
                    ...
                    [self adLoaded];
                    self->mediation = self->_rewardedInterstitialAd.responseInfo.loadedAdNetworkResponseInfo ? self->_rewardedInterstitialAd.responseInfo.loadedAdNetworkResponseInfo.adNetworkClassName : @"admob";
            ...
            [self logEvent:@"sdk_ad_request" provider:@"admob" placement:_adId mediation:NULL];
            _rewardedLoadStartTime = [[NSDate date] timeIntervalSince1970] * 1000.0;
            [GADRewardedAd loadWithAdUnitID: _adId request:request completionHandler:^(GADRewardedAd *ad, NSError *error) {
                ...
                [self adLoaded];
                self->mediation = self->_rewardedAd.responseInfo.loadedAdNetworkResponseInfo ? self->_rewardedAd.responseInfo.loadedAdNetworkResponseInfo.adNetworkClassName : @"admob";
                NSTimeInterval duration = [[NSDate date] timeIntervalSince1970] * 1000.0 - self->_rewardedLoadStartTime;
//                [self logEvent2:@"video_fill_duration" provider:@"admob" placement:self->_adId mediation:self->mediation duration:[NSNumber numberWithDouble:duration]];
                [self logEvent:@"sdk_ad_loaded" provider:@"admob" placement:self->_adId mediation:self->mediation];
...
-(void)logEvent:(NSString *)eventId provider:(NSString* )provider placement:(NSString *)placement mediation:(NSString *)mediation
{
    [self logEvent2:eventId provider:provider placement:placement mediation:mediation duration:NULL];
}

-(void)logEvent2:(NSString *)eventId provider:(NSString* )provider placement:(NSString *)placement mediation:(NSString *)mediation duration:(id) duration
{
    NSMutableDictionary * dict = [[NSMutableDictionary alloc] init];
    [dict setObject:provider forKey:@"ad_network"];
    [dict setObject:@"rewarded" forKey:@"ad_format"];
    [dict setObject:placement forKey:@"ad_unit"];
    if (mediation) {
        [dict setObject:mediation forKey:@"mediation"];
    }
    if (duration > 0) {
        [dict setObject:duration forKey:@"duration"];
    }
    NSString * netStatus = [[SDKFacade sharedInstance] isNetworkConnected] ? @"1" : @"0";
    [dict setObject:netStatus forKey:@"sdkNetStatus"];
    [[SDKFacade sharedInstance] logEvent:eventId withData:dict];
}
