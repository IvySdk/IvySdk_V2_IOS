...
//    [SDKFacade sharedInstance].backHomeAdTag = @"custom";
    ...
    btnNames = @[@"0)Start AD", @"1)Pause AD", @"2)Custom AD", @"3)Reward Video", @"4)ShowBanner",
                 ...
                 @"59)unknown"];
//    CGSize _size = CGSizeMake(Screen_width, Screen_height + (btnNames.count / 2 - Screen_height / 60) * 80);

    self.ScrollView.contentSize = CGSizeMake(Screen_width, Screen_height + (btnNames.count / 2 - Screen_height / 60) * 80);
...
-(void)uiButtonPressed:(UIButton*)button {
    ...
    switch(idx) {
        ...
        case 1://Pause AD
            ...
            break;
        case 2://Custom AD
            isPushRegisted = [[SDKFacade sharedInstance] isPushRegistered];
            str = [NSString stringWithFormat:@"push register::%@", isPushRegisted ? @"1" : @"0"];
            NSLog(@"%@", str);
            [[SDKFacade sharedInstance] toast:str isLongTime:FALSE];
...
//             });
        ...
        case 11:
            ...
            break;
        case 12:
            [[SDKFacade sharedInstance] pay:100];
            break;
        case 13:
